...
import type {RecommendedOutfitDto, WeatherDto} from '@/lib/api';
...
export default function AddOutfitModal({open, outfit, weather, onClose}: AddOutfitModalProps) {
  const {selectedWeather} = useWeatherStore();
  const outfitWeather = weather ?? selectedWeather;
  ...
  const handleSubmit = async () => {
    if (!outfit || !outfitWeather || !name.trim()) {
      ...
      <DialogContent className="w-[520px] max-w-[calc(100%-2rem)] rounded-[24px] bg-white p-7 sm:max-w-[calc(100%-2rem)]" showCloseButton={false}>
        <div className="flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <h2 className="font-extrabold text-[#212126] text-[22px]">아웃핏 등록하기</h2>
            <button type="button" onClick={onClose} className="rounded-[8px] px-2 py-1 font-semibold text-[#808089] hover:bg-[#f7f7f8]">닫기</button>
          </div>
          <p className="text-[#696975] text-[14px]">현재 추천 코디의 옷 {outfit?.clothes.length ?? 0}개가 아웃핏으로 저장됩니다.</p>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button type="button" onClick={handleSubmit} disabled={loading || !name.trim()} className="h-[46px] rounded-[10px] bg-[#1e89f4] px-5 font-bold text-white disabled:opacity-50">{loading ? '등록 중...' : '아웃핏 등록'}</button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
